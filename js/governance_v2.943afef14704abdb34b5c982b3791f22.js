...
async function main() {
  ...
  $("#my_votes").html((currentXSNOB / 1e18).toFixed(2).toLocaleString())
  ...
  for (let i = proposal_count * 1; i > 0; i--) {
    ...
    if (state == 0 && userVoted == false) {
      ...
      proposal_html += `<div class="ml-20"><span>Your vote: ${userVoted == 0 ? 'Did not vote' : userVoteDisplay} </span></div>`;
    }
