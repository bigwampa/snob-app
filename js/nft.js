...
async function main() {
    ...
    const GOVERNANCE_ABI = [
        {
            ...
            inputs: [
                ...
                { type: 'address', name: '_governer', internalType: 'address' },
            ],
        },
        {
            type: 'event',
    ...
    const COVID_RELIEF_ADDRESS = '0xD928Ab4b54F7FD0498160Ee52AC0C92BbB9C9cb3'

    const covidRelief_claim = async function() {
        return covidReliefContract_claim(
            COVID_RELIEF_ABI,
            COVID_RELIEF_ADDRESS,
            App
        )
    }

    const COVID_RELIEF_CONTRACT = new ethers.Contract(
        COVID_RELIEF_ADDRESS,
        COVID_RELIEF_ABI,
        signer
    )
    const covidReliefBalance = await COVID_RELIEF_CONTRACT.balanceOf(
        App.YOUR_ADDRESS
    )
    const covidReliefEligible = await COVID_RELIEF_CONTRACT.Wallets(
        App.YOUR_ADDRESS
    )

    if (covidReliefEligible == true) {
        $('#covid_relief_status').html('Eligible')
        if (covidReliefBalance == 0) {
            $('#claim_covid_relief').show()
        } else {
            $('#claim_covid_relief_claimed').show()
        }
    } else {
        $('#covid_relief_status').html('Did not donate')
    }

    if (covidReliefBalance > 0) {
        printNFT(
            'Covid Relief in India',
            'https://gateway.pinata.cloud/ipfs/QmfRecmhvomvhpR1HVnXrmHNBEfgGcR4Coc1gWSJb6au5s',
            'Covid Relief in India',
            'Donated to the Snowball campaign for Covid Relief in India',
            null,
            null,
            null,
            'gray'
    ...
    const SNOWBALL_HEAD_CONTRACT = new ethers.Contract(
        SNOWBALL_HEAD_ADDRESS,
        SNOWBALL_HEAD_ABI,
        signer
    )
    ...
    const sh_tokensOfOwner = await SNOWBALL_HEAD_CONTRACT.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    ...
    printRolling({
        ...
        name: 'Snow Ball Head',
        img_url:
            'https://gateway.pinata.cloud/ipfs/QmZnucy9uL9nLD8GVZHbVSUVgMkj1ThTvT6jFm713uNe6e',
    ...
    const ROLLING_CONTRACT_PINK = new ethers.Contract(
        ROLLING_ADDR_PINK,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_PINK = await ROLLING_CONTRACT_PINK.totalSupply()
    const rollingBalance_PINK = await ROLLING_CONTRACT_PINK.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_PINK = await ROLLING_CONTRACT_PINK.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    ...
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Pink.gif',
    ...
    const ROLLING_CONTRACT_PURPLE = new ethers.Contract(
        ROLLING_ADDR_PURPLE,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_PURPLE = await ROLLING_CONTRACT_PURPLE.totalSupply()
    const rollingBalance_PURPLE = await ROLLING_CONTRACT_PURPLE.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_PURPLE = await ROLLING_CONTRACT_PURPLE.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Purple.gif',
    ...
    const ROLLING_CONTRACT_ORANGE = new ethers.Contract(
        ROLLING_ADDR_ORANGE,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_ORANGE = await ROLLING_CONTRACT_ORANGE.totalSupply()
    const rollingBalance_ORANGE = await ROLLING_CONTRACT_ORANGE.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_ORANGE = await ROLLING_CONTRACT_ORANGE.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Orange.gif',
    ...
    const ROLLING_CONTRACT_BLUE = new ethers.Contract(
        ROLLING_ADDR_BLUE,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_BLUE = await ROLLING_CONTRACT_BLUE.totalSupply()
    const rollingBalance_BLUE = await ROLLING_CONTRACT_BLUE.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_BLUE = await ROLLING_CONTRACT_BLUE.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Blue.gif',
    ...
    const ROLLING_CONTRACT_GREEN = new ethers.Contract(
        ROLLING_ADDR_GREEN,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_GREEN = await ROLLING_CONTRACT_GREEN.totalSupply()
    const rollingBalance_GREEN = await ROLLING_CONTRACT_GREEN.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_GREEN = await ROLLING_CONTRACT_GREEN.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Green.gif',
    ...
    const ROLLING_CONTRACT_LASER = new ethers.Contract(
        ROLLING_ADDR_LASER,
        ROLLING_ABI,
        signer
    )
    const rolling_minted_LASER = await ROLLING_CONTRACT_LASER.totalSupply()
    const rollingBalance_LASER = await ROLLING_CONTRACT_LASER.balanceOf(
        App.YOUR_ADDRESS
    )
    const tokensOfOwner_LASER = await ROLLING_CONTRACT_LASER.tokensOfOwner(
        App.YOUR_ADDRESS
    )
    printRolling({
        ...
        img_url:
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/Snowball_NFT_Sasquatch.gif',
    ...
    const earlyVote_claim = async function() {
        return earlyVoteContract_claim(
            EARLY_VOTER_ABI,
            EARLY_VOTER_ADDRESS,
            App
        )
    ...
    const EARLY_VOTER_CONTRACT = new ethers.Contract(
        EARLY_VOTER_ADDRESS,
        EARLY_VOTER_ABI,
        signer
    )
    const userEarlyVoteBalance = await EARLY_VOTER_CONTRACT.balanceOf(
        App.YOUR_ADDRESS
    )
    ...
    if (userEarlyVoteBalance > 0) {
        printNFT(
            'Early Voter',
            'https://raw.githubusercontent.com/Snowball-Finance/Assets/main/nft1-3.gif',
            'Sasquatch throwing snowball',
            'Voted on Proposal 1 or Proposal 2 during the first week of Snowball Governance',
            null,
            null,
            null,
            null
    ...
    // deposit/withdraw
    $('#claim_covid_relief').click(function() {
        covidRelief_claim()
    })
...
function printShopItem(
    ...
    let html = `<div class="card col-md-3">`
...
const covidReliefContract_claim = async function(abi, address, App) {
    const signer = App.provider.getSigner()
    console.log(signer)
    ...
    allow
        .then(async function() {
            NFT_CONTRACT.mint(App.YOUR_ADDRESS)
                .then(function(t) {
                    App.provider.waitForTransaction(t.hash).then(function() {
                        hideLoading()
                        alert('NFT Claimed. Refresh page to NFTs')
